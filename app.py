...
def red(message):
    server_address = "http://127.0.0.1:4000/request"
...
def register(username, password, email):
    ...
    response = red(message)

    if isinstance(response, dict):  # Check if the response is a dictionary
        if "reason" in response:
            return -1  # Registration failed due to some reason
        elif response.get("code") == "fail":
            return -2  # Registration failed
        else:
            return 1  # Registration successful
    else:
        return -3  # Unexpected response format or server error
